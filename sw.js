...
const VERSION = 'kairos-v5';
const CORE = ['./', './index.html', './contenu.js', './meta.js', './manifest.webmanifest', './icon-192.png', './icon-512.png', './icon-512-maskable.png'];
...
self.addEventListener('fetch', e => {
  ...
  const isData = url.origin === location.origin && /\/(contenu|meta)\.js$/.test(url.pathname);
  if (isPage || isData) {
